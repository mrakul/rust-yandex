...
Весь проект в корне - в виде Workspace, Cargo.toml которого определяет 4 крата и содержит зависимости по всем кратам.
...
На уровне Application содержит сервис авторизации AuthSerivce (JWT) и BlogService с сценариями использования - операции с постами.
...
От интерфейса потрясающего user experience ожидать не стоит, надеюсь на понимание, я не фронтендер (последнее, что трогал - это JQuery лет 10 назад).
...
health:
m_rakul@NB1729:~/Code/rust-yandex/practice/module_3_blog$ curl http://127.0.0.1:3000/api/health
...
Можно посмотреть токен: echo $TOKEN
...
Можно сверить, что токен другой: echo $TOKEN
...
Создание поста:
curl -v -X POST http://127.0.0.1:3000/api/posts   -H "Content-Type: application/json"   -H "Authorization: Bearer $TOKEN"   -d '{
    "title": "Изначальный пост пользователя yandex",
    "content": "Изначальный заголовок"
  }'

> Authorization: Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIxMSIsInVzZXJuYW1lIjoieWFuZGV4IiwiZXhwIjoxNzgxMDQ4MzkzLCJpc3N1ZWRfYXQiOjE3ODA5NjE5OTN9.kcJHfo0WOylOUKBzbHJfRtpKSbBGPfJHjBHmKuXEikE
> Content-Length: 144
> 
< HTTP/1.1 201 Created
< content-length: 244
...
Изменение поста:
curl -X PUT http://127.0.0.1:3000/api/posts/19   -v -H "Content-Type: application/json"   -H "Authorization: Bearer $TOKEN"   -d '{
...
=>
{
  "token": "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIxMyIsInVzZXJuYW1lIjoieWFuZGV4MyIsImV4cCI6MTc4MTA0OTgwOCwiaXNzdWVkX2F0IjoxNzgwOTYzNDA4fQ.lb8y9v2Jmm6tgrK4yb7f2JBVZRJIWLndd9Df4fvSPxY",
  "user": {
    "id": "13",
    "username": "yandex3",
    "email": "yandex3@yandex.ru",
...
=>
{
  "token": "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIxMyIsInVzZXJuYW1lIjoieWFuZGV4MyIsImV4cCI6MTc4MTA1MDI1NCwiaXNzdWVkX2F0IjoxNzgwOTYzODU0fQ._pl8306R6LE40Fyda191xuzCp6t0jSnx6Fd2d1_qoqs",
  "user": {
    "id": "13",
    "username": "yandex3",
    "email": "yandex3@yandex.ru",
...
export TOKEN=$(grpcurl -plaintext \
...
grpcurl -plaintext   -proto proto/blog.proto   -H "authorization: Bearer BAD_STRING"   -d '{
    "title": "Пост через gRPC",
    "content": "Текст через gRPC"  
  }'   localhost:50051 blog.BlogService/CreatePost
ERROR:
  ...
  }'   localhost:50051 blog.BlogService/ListPosts
{
  "posts": [
    ...
    },
    {
      ...
      "content": "Test 3",
      "authorId": "4",
      "createdAt": "2026-06-06T06:10:38.151534+00:00",
      "updatedAt": "2026-06-07T04:22:49.460542+00:00"
    },
...
3. Описание frontend приведено в описании крата, тут лучше уже всё проверить.
