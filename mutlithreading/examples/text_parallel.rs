...
fn fix(bytes_to_fix: &str, set_of_labels: &HashSet<&str>, id: usize) -> String {
...
// Создаёт 4 scope-потока
fn run_parallel(string: String, set_of_labels: &HashSet<&str>) -> String {
    std::thread::scope(
        |scope| {
            let threads_count = 4;
            let parts = split_n(&string, threads_count);
            
            let (res1, res2, res3, res4) = std::thread::scope(
                     |scope| {
                         let r1 = scope.spawn(|| fix(&parts[0], set_of_labels, 0));
                         let r2 = scope.spawn(|| fix(&parts[1], set_of_labels, 1));
                         let r3 = scope.spawn(|| fix(&parts[2], set_of_labels, 2));
                         let r4 = scope.spawn(|| fix(&parts[3], set_of_labels, 3));
                         // нельзя обратиться по &mut self, когда выше есть просто &
                         //label1.clear();
                         //label2.clear();
                         println!("from main thread");
                         (r1.join().unwrap(), r2.join().unwrap(), r3.join().unwrap(), r4.join().unwrap())
                     }
                 );

            let mut result = res1 + &res2 + &res3 + &res4;
...
fn main() {
    ...
    let dict_hash_set = HashSet::from(dict);
    let thread = std::thread::spawn(move||run_parallel(mix.to_string(), &dict_hash_set));
    let processed_text = thread.join().unwrap();
    
    println!("{}", processed_text);
    assert_eq!(processed_text, orig);
}